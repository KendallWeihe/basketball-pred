...
for i in range(actual_spreads.shape[0]):
    if 4 <= math.fabs(actual_spreads[i] - spreads1[i]) <= 12 and\
        2 <= math.fabs(actual_spreads[i] - spreads2[i]) <= 6:
...
for i in range(my_spreads.shape[0]):
    ...
    actual_spread = my_spreads[i,1]

    if (my_spreads[i,0] > vegas_spreads[i,0] and actual_spread > vegas_spreads[i,0]) or\
        (my_spreads[i,0] < vegas_spreads[i,0] and actual_spread < vegas_spreads[i,0]):

        my_correct_count = my_correct_count + 1

    if my_spreads[i,0] != vegas_spreads[i,0]:
        my_count = my_count + 1
